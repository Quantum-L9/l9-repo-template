...
| Gate | Command | Proves |
|------|---------|--------|
| Inventory | `make inventory-check` | Required baseline files, deny dirs, no Gate-worker defaults, tools allowlist, sanctioned CodeQL caller, agent/security discovery wiring |
...
`l9-ci-core` security CI installs Gitleaks and automatically uses
`.gitleaks.toml` when that file exists. The root `bootstrap.sh` delegates to the
existing `tools.l9_repo setup` command; it does not create a second bootstrap
implementation.
...
- GitHub-hosted CodeQL execution or Security-tab upload; that requires a real Actions run
